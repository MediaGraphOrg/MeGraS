...
import org.megras.util.services.SigLipEmbedderClient

class SigLipEmbeddingHandler(private val quadSet: QuadSet, private val objectStore: FileSystemObjectStore) : DerivedRelationHandler<FloatVectorValue> {
    override val predicate: URIValue = getPredicate()
    override val requiresExternalService: Boolean = true

    companion object {
        fun getPredicate(): URIValue {
            return URIValue("${Constants.DERIVED_PREFIX}/siglipEmbedding")
        }
    }
    ...
    override fun derive(subject: URIValue): Collection<FloatVectorValue> {
        ...
        val embedding: List<Float> = runBlocking {
            val client = SigLipEmbedderClient(ServiceConfig.grpcHost, ServiceConfig.grpcPort)
            try {
