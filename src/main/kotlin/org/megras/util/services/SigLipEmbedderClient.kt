...
import org.megras.util.services.SiglipServiceGrpcKt.SiglipServiceCoroutineStub
 ...
 */
class SigLipEmbedderClient(private val channel: ManagedChannel) : Closeable {

    private val stub: SiglipServiceCoroutineStub = SiglipServiceGrpcKt.SiglipServiceCoroutineStub(channel)
    ...
    suspend fun getImageEmbedding(imagePath: String): List<Float> {
        try {
            ...
            val request = ImageRequest.newBuilder().setImageData(ByteString.copyFrom(imageBytes)).build()
